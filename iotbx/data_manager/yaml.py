...
class YamlDataManager(DataManagerBase):
  ...
  def process_yaml_file(self, filename):
    ...
    Sorry
        ...
        is not valid YAML
    '''
    if filename not in self.get_yaml_names():
      if not os.path.isfile(filename):
        raise Sorry("Couldn't find the file %s" % filename)
      yaml = self._yaml_module()  # outside the try: a missing-PyYAML Sorry
                                  # must NOT be masked as "not a valid YAML file"
      try:
        with open(filename, 'r', encoding='utf-8') as f:
          obj = yaml.safe_load(f)
      except Exception:
        # only structurally malformed YAML reaches here (most plain text is
        # valid YAML); convert yaml.YAMLError into a user-facing Sorry
        raise Sorry('%s is not a valid YAML file' % filename)
      self.add_yaml(filename, obj)
    return filename
