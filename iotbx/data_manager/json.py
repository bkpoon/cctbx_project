...
class JsonDataManager(DataManagerBase):
  ...
  def process_json_file(self, filename):
    ...
    '''
    if filename not in self.get_json_names():
      ...
      try:
        ...
        # json.load raises JSONDecodeError (a ValueError); convert any
        # parse/read failure into a user-facing Sorry
        raise Sorry('%s is not a valid JSON file' % filename)
      self.add_json(filename, obj)
    return filename
